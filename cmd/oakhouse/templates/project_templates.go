...
// PaginationScope creates a GORM scope for pagination
...
// DateRangeFilter creates a GORM scope for filtering {{.ModelName}}s by creation date range
func DateRangeFilter(dto *{{.PackageName}}.Get{{.ModelName}}Dto) func(*gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		// Apply start date filter if provided
		if !dto.StartDate.IsZero() {
			db = db.Where("created_at >= ?", dto.StartDate)
		}
		
		// Apply end date filter if provided
		if !dto.EndDate.IsZero() {
			db = db.Where("created_at <= ?", dto.EndDate)
		}
